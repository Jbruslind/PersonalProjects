...
#include "modbus_params.h"      // for modbus parameters structure
...
// #define CONFIG_ETH_RMII_CLK_OUT_GPIO    GPIO_NUM_17
...
/**
 * @brief Main function for starting ethernet driver and binding to Netif
 *
 * @return netif object
 */
...
static esp_netif_t *eth_start(void)
{
    ...
    esp_eth_config_t config = ETH_DEFAULT_CONFIG(mac, s_phy);
...
/**
 * @brief Initial function that starts ethernet and creates tcp task
 *
 * @return ESP_OK
 */
static esp_err_t example_ethernet_connect(void)
{
...
static esp_err_t init_services(void)
{
    ...
    result = example_ethernet_connect();
    MB_RETURN_ON_FALSE((result == ESP_OK), ESP_ERR_INVALID_STATE,
                                TAG,
